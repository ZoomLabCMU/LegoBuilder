...
from LegoDetector import Detectron2

import random
from utils import save_rgb, save_proprioceptive
...
class LegoBuilderControlNode(Node):
    def __init__(self):
        super().__init__('Lego Builder Control Node') # type: ignore

        # Workspace camera subscriber
        self.workspace_camera_sub = self.create_subscription(
            Image,
            "workspace_img",
            self.recv_workspace_camera,
            10
        )
    ...
    def recv_workspace_camera(self, img : Image):
        return None
    ...
    for i in range(args.num_trials):
        for skill in skill_actions:
            ...
            if random_direction == 0:
                ...
                print(skill + ' ' + str(num_blocks) + ' blocks using back tilt.')

                if not os.path.exists(directory_path + skill + '/back/' + str(starting_top)):
                    os.makedirs(directory_path + skill + '/back/' + str(starting_top))
                if not os.path.exists(directory_path + skill + '/back/' + str(starting_top) + '/' + str(starting_bottom)):
                    os.makedirs(directory_path + skill + '/back/' + str(starting_top) + '/' + str(starting_bottom))
                if not os.path.exists(directory_path + skill + '/back/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks)):
                    os.makedirs(directory_path + skill + '/back/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks))

                path = directory_path + skill + '/back/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks) + '/'
                folder_num = str(len(os.listdir(path)))
                os.mkdir(path+folder_num)
                ...
                # Left
                #random_y_offset = random.uniform(-0.005, 0.005)
                random_y_offset = 0
                
                starting_x = 0
                starting_y = block_length/2

                print(skill + ' ' + str(num_blocks) + ' blocks using left tilt.')

                if not os.path.exists(directory_path + skill + '/left/' + str(starting_top)):
                    os.makedirs(directory_path + skill + '/left/' + str(starting_top))
                if not os.path.exists(directory_path + skill + '/left/' + str(starting_top) + '/' + str(starting_bottom)):
                    os.makedirs(directory_path + skill + '/left/' + str(starting_top) + '/' + str(starting_bottom))
                if not os.path.exists(directory_path + skill + '/left/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks)):
                    os.makedirs(directory_path + skill + '/left/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks))

                path = directory_path + skill + '/left/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks) + '/'
                folder_num = str(len(os.listdir(path)))
                os.mkdir(path+folder_num)
                ...
                trial_dict = {'block_pose':block_pose,
                              ...
                              'direction':'left',
                              'num_blocks':num_blocks,
                              'tcp':[starting_x, starting_y + random_y_offset, starting_z + random_z_offset],
                              'random_offset':[0, random_y_offset, random_z_offset]}
                ...
                ur_controller.set_tcp([starting_x, starting_y + random_y_offset, starting_z + random_z_offset, 0, 0, 0])

                ur_controller.rotate_ee_degrees([0,1,0], -random_angle, 3, force_thresholds=[80,80,80])
                ...
                print(skill + ' ' + str(num_blocks) + ' blocks using right tilt.')

                if not os.path.exists(directory_path + skill + '/right/' + str(starting_top)):
                    os.makedirs(directory_path + skill + '/right/' + str(starting_top))
                if not os.path.exists(directory_path + skill + '/right/' + str(starting_top) + '/' + str(starting_bottom)):
                    os.makedirs(directory_path + skill + '/right/' + str(starting_top) + '/' + str(starting_bottom))
                if not os.path.exists(directory_path + skill + '/right/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks)):
                    os.makedirs(directory_path + skill + '/right/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks))

                path = directory_path + skill + '/right/' + str(starting_top) + '/' + str(starting_bottom) + '/' + str(num_blocks) + '/'
                folder_num = str(len(os.listdir(path)))
                os.mkdir(path+folder_num)
