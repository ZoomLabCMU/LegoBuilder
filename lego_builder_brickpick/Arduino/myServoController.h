...
public:
  ...
  // State estimation
  float _joint_lengths[NUM_MOTORS];
  float _joint_limits[NUM_MOTORS][2];
...
private:
  ...
  const float _cmdPos_Kp[NUM_MOTORS] = {1200, 1200, 1200, 0};
  const float _cmdPos_Ki[NUM_MOTORS] = {0, 0, 0, 0};
  const float _cmdPos_Kd[NUM_MOTORS] = {50, 30, 50, 0};
